...
private:
    ...
    bool pos = false; // true if number is positive
...
{
    //PROFINY_SCOPE
    val.push_back(static_cast<ELEM_TYPE>(0));
}
...
inline InfInt::InfInt(int l) : pos(l >= 0)
{
    ...
    do
    {
        ...
        val.push_back(static_cast<ELEM_TYPE>(dt.rem));
        l = dt.quot;
    } while (l > 0);
...
inline InfInt::InfInt(unsigned int l) : pos(true)
{
    ...
    do
    {
        val.push_back(static_cast<ELEM_TYPE>((l % BASE)));
        l = l / BASE;
    } while (l > 0);
}
...
inline const InfInt& InfInt::operator/=(const InfInt& rhs)
{
    ...
    for (int i = static_cast<int>(N.val.size()) - 1; i >= 0; --i)
    {
...
inline InfInt InfInt::operator*(const InfInt& rhs) const
{
    ...
    for (;; ++digit)
    {
        ...
        carry = dt.quot;
        result.val[digit] = static_cast<ELEM_TYPE>(dt.rem);

        bool found = false;
        for (size_t i = digit < rhs.val.size() ? 0 : digit - rhs.val.size() + 1; i < val.size() && i <= digit; ++i)
        {
            PRODUCT_TYPE pval = result.val[digit] + val[i] * static_cast<PRODUCT_TYPE>(rhs.val[digit - i]);
            if (pval >= BASE || pval <= -BASE)
            {
                lldiv_t cdt = my_lldiv(pval, BASE);
                carry += cdt.quot;
                pval = cdt.rem;
            }
            result.val[digit] = static_cast<ELEM_TYPE>(pval);
            found = true;
        }
        if (!found)
        {
...
inline void InfInt::optimizeSqrtSearchBounds(InfInt& lo, InfInt& hi) const
{
    //PROFINY_SCOPE
    InfInt hdn = 1;
    for (int i = static_cast<int>(this->numberOfDigits()) / 2; i >= 2; --i)
    {
        hdn *= 10;
    }
    if (lo < hdn)
...
inline bool InfInt::equalizeSigns()
{
    ...
    int i = static_cast<int>(val.size()) - 1;
    for (; i >= 0; --i)
    {
        if (val[i] != 0)
        {
    ...
    if (isPositive)
    {
        for (; i >= 0; --i)
        {
            ...
            {
                int k = 0, index = i + 1;
                for (; static_cast<size_t>(index) < val.size() && val[index] == 0; ++k, ++index)
...
{
    //PROFINY_SCOPE
    for (int i = static_cast<int>(val.size()) - 1; i > 0; --i) // remove leading 0's
    {
        if (val[i] != 0)
        {
            return;
...
inline void InfInt::fromString(const std::string& s)
{
    ...
    int i = static_cast<int>(s.size()) - DIGIT_COUNT;
...
inline void InfInt::multiplyByDigit(ELEM_TYPE factor, std::vector<ELEM_TYPE>& val)
{
    ...
    for (size_t i = 0; i < val.size(); ++i)
    {
        PRODUCT_TYPE pval = val[i] * static_cast<PRODUCT_TYPE>(factor) + carry;
        if (pval >= BASE || pval <= -BASE)
        {
            lldiv_t dt = my_lldiv(pval, BASE);
            carry = static_cast<ELEM_TYPE>(dt.quot);
            pval = dt.rem;
        }
        else
        {
        ...
        val[i] = static_cast<ELEM_TYPE>(pval);
    }
    if (carry > 0)
    {
        val.push_back(carry);
