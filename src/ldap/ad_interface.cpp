...
{
    auto error_message = [&](const QString &error) { d->error_message(tr("Failed to create GPO."), error); };
    ...
    const QString filesys_path = QString("\\\\%1\\sysvol\\%2\\Policies\\%3")
...
bool AdInterface::gpo_sync_perms(const QString &dn)
{
    ...
    {
        const int set_sd_result = smbc_setxattr(cstr(path),
                                                "system.nt_sec_desc.*",
                                                cstr(gpt_sd_string),
...
QString get_gpt_sd_string(const AdObject &gpc_object, const AceMaskFormat format_enum)
{
    ...
    for (uint32_t i = 0; i < gpt_sd->dacl->num_aces; i++)
    {
        ...
        all_elements.append(
            QString("ACL:%1:%2/%3/%4")
                .arg(trustee_string, QString::number(ace.type), QString::number(ace.flags), access_mask_string));
    }
