...
{
    return (a.keywordEnabled == b.keywordEnabled && a.titleEnabled == b.titleEnabled && a.helpEnabled == b.helpEnabled
            && a.commentEnabled == b.commentEnabled && a.keywordType == b.keywordType && a.keywordText == b.keywordText
...
char *toString(const TemplateFilter &filter)
{
    ...
    const QString configuredString = [&]() {
        ...
        for (const PolicyStateManager::PolicyState &state : filter.configured)
        {
            const QString stateString = [&]() {
                switch (state)
                {
                case PolicyStateManager::STATE_NOT_CONFIGURED:
                    return "NOT CONFIGURED";
                case PolicyStateManager::STATE_ENABLED:
                    return "ENABLED";
                case PolicyStateManager::STATE_DISABLED:
                    return "DISABLED";
                }
                return "";
            }();
    ...
    const QString string = QString("\nkeywordEnabled = %1\ntitleEnabled = %2\nhelpEnabled = %3\ncommentEnabled = "
                                   "%4\nkeywordType = %5\nkeywordText = %6\nconfigured = %7")
                               .arg(filter.keywordEnabled)
                               .arg(filter.titleEnabled)
                               .arg(filter.helpEnabled)
                               .arg(filter.commentEnabled)
                               .arg(keywordTypeString)
                               .arg(filter.keywordText)
                               .arg(configuredString);
...
void TemplateFilterTest::filterAcceptsRow_data()
{
    ...
    QTest::newRow("filter disabled") << TemplateFilter() << false << false << ""
                                     << ""
                                     << "" << 0 << true;
    QTest::newRow("not template") << TemplateFilter() << true << false << ""
                                  << ""
                                  << "" << 0 << true;

    {
        ...
        QTest::newRow("configured yes, accept enabled") << filter << true << true << ""
                                                        << ""
                                                        << "" << (int) PolicyStateManager::STATE_ENABLED << true;
        QTest::newRow("configured yes, accept disabled") << filter << true << true << ""
                                                         << ""
                                                         << "" << (int) PolicyStateManager::STATE_DISABLED << true;
        QTest::newRow("configured yes, accept not configured")
            << filter << true << true << ""
            << ""
            << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
    }

    {
        ...
        QTest::newRow("configured yes, accept disabled") << filter << true << true << ""
                                                         << ""
                                                         << "" << (int) PolicyStateManager::STATE_DISABLED << true;
        QTest::newRow("configured yes, reject not configured")
            << filter << true << true << ""
            << ""
            << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;
    }

    {
        ...
        QTest::newRow("configured no, reject enabled") << filter << true << true << ""
                                                       << ""
                                                       << "" << (int) PolicyStateManager::STATE_ENABLED << false;
        QTest::newRow("configured no, reject disabled") << filter << true << true << ""
                                                        << ""
                                                        << "" << (int) PolicyStateManager::STATE_DISABLED << false;
        QTest::newRow("configured no, accept not configured")
            << filter << true << true << ""
            << ""
            << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
    }

    {
        ...
        QTest::newRow("filter any, accept hello") << filter << true << true << "hello"
                                                  << ""
                                                  << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter any, accept sailor") << filter << true << true << "sailor"
                                                   << ""
                                                   << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter any, reject modernity") << filter << true << true << "modernity"
                                                      << ""
                                                      << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;
    }

    {
        ...
        QTest::newRow("filter exact, accept") << filter << true << true << "hello sailor"
                                              << ""
                                              << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter exact, reject") << filter << true << true << "goodbye sailor"
                                              << ""
                                              << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;
    }

    {
        ...
        QTest::newRow("filter all, accept same string") << filter << true << true << "hello sailor"
                                                        << ""
                                                        << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter all, accept extra word") << filter << true << true << "hello brave sailor"
                                                       << ""
                                                       << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter all, reject missing word")
            << filter << true << true << "goodbye sailor"
            << ""
            << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;
    }

    {
        ...
        QTest::newRow("filter exact, reject mismatch") << filter << true << true << "goodbye sailor"
                                                       << ""
                                                       << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;

        filter.titleEnabled   = true;
        filter.keywordEnabled = false;
        QTest::newRow("filter exact, accept mismatch because keyword filter is disabled")
            << filter << true << true << "goodbye sailor"
            << ""
            << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
    }

    {
        ...
        QTest::newRow("filter exact, accept title, help and comment")
            << filter << true << true << "hello sailor"
            << "hello sailor"
            << "hello sailor" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter exact, accept partial match of title/comment")
            << filter << true << true << "hello sailor"
            << "goodbye sailor"
            << "hello sailor" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << true;
        QTest::newRow("filter exact, reject no match") << filter << true << true << "goodbye sailor"
                                                       << "goodbye sailor"
                                                       << "" << (int) PolicyStateManager::STATE_NOT_CONFIGURED << false;
    }
}
