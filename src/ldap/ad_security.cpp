...
bool check_ace_match(const security_ace &ace,
                     const QByteArray &trustee,
                     const QByteArray &object_type,
                     const bool allow,
    ...
    const bool enabled = [&]() {
        ...
        for (const QString &trustee_cn : cant_change_pass_trustee_cn_list)
        {
            const bool is_denied = [&]() {
                ...
                const SecurityRightState state     = security_descriptor_get_right(sd,
                                                                               trustee,
...
SecurityRightState security_descriptor_get_right(const security_descriptor *sd,
                                                 const QByteArray &trustee,
                                                 const uint32_t access_mask_arg,
                                                 const QByteArray &object_type)
{
                                   ...
                                   const QLocale::Language language)
{
    ...
    else
    {
        ...
        return common_right_name_map.value(access_mask,
                                   ...
                                   const bool allow)
{
    ...
    for (const SecurityRight &superior : superior_list)
    {
        const bool opposite_superior_is_set = [&]() {
            const SecurityRightState state    = security_descriptor_get_right(sd,
                                                                           trustee,
                                                                           superior.access_mask,
                                                                           superior.object_type);
        ...
        const QList<SecurityRight> superior_subordinate_list
            = ad_security_get_subordinate_right_list(adconfig, superior.access_mask, superior.object_type, class_list);
    ...
    // Remove subordinates
    const QList<SecurityRight> subordinate_list = ad_security_get_subordinate_right_list(adconfig,
                                                                                         access_mask,
                                                                                         object_type,
                                                                                         class_list);
                                      ...
                                      const bool allow)
{
    ...
    const QList<SecurityRight> tarad_security_get_subordinate_right_list
        = ad_security_get_subordinate_right_list(adconfig, access_mask, object_type, class_list);
...
QList<SecurityRight> ad_security_get_subordinate_right_list(AdConfig *adconfig,
                                                            const uint32_t access_mask,
                                                            const QByteArray &object_type,
                                                            const QList<QString> &class_list)
{
...
int ace_compare_simplified(const security_ace &ace1, const security_ace &ace2)
{
    ...
    b1 = (ace1.type == SEC_ACE_TYPE_ACCESS_ALLOWED || ace1.type == SEC_ACE_TYPE_ACCESS_ALLOWED_OBJECT);
    b2 = (ace2.type == SEC_ACE_TYPE_ACCESS_ALLOWED || ace2.type == SEC_ACE_TYPE_ACCESS_ALLOWED_OBJECT);
    if (b1 != b2)
    {
        return (b1 ? 1 : -1);
    }
