...
// Q_UNUSED(group);
    ...
    for (const auto& groupsSchema : group->Group())
    {
        ...
        for (const auto& properties: groupsSchema.Properties())
        {
            ...
            localGroup.setProperty<std::string>(LocalGroupItem::USER_ACTION, getOptionalPropertyData(properties.userAction()));
            localGroup.setProperty<std::string>(LocalGroupItem::DESCRIPTION, getOptionalPropertyData(properties.description()));
            localGroup.setProperty<std::string>(LocalGroupItem::NEW_NAME, getOptionalPropertyData(properties.newName()));
            localGroup.setProperty<std::string>(LocalGroupItem::USER_ACTION, getOptionalPropertyData(properties.userAction()));
            localGroup.setProperty<bool>(LocalGroupItem::REMOVE_ACCOUNTS, getOptionalPropertyData(properties.removeAccounts()));
            localGroup.setProperty<bool>(LocalGroupItem::DELETE_ALL_USERS, getOptionalPropertyData(properties.deleteAllUsers()));
            localGroup.setProperty<bool>(LocalGroupItem::DELETE_ALL_GROUPS, getOptionalPropertyData(properties.deleteAllGroups()));
    ...
    for (const auto& userSchema : group->User())
    {
        ...
        for (const auto& properties : userSchema.Properties())
        {
            ...
            auto action = properties.action();
            auto state = getActionCheckboxState(action.present() ? action->c_str() : "");
            localUser.setProperty<std::string>(LocalUserItem::ACTION, state);
            ...
            localUser.setProperty<std::string>(LocalUserItem::DESCRIPTION, getOptionalPropertyData(properties.description()));
            localUser.setProperty<std::string>(LocalUserItem::CPASSWORD, getOptionalPropertyData(properties.cpassword()));
            localUser.setProperty<bool>(LocalUserItem::CHANGE_LOGON, getOptionalPropertyData(properties.changeLogon()));
            localUser.setProperty<bool>(LocalUserItem::ACCOUNT_DISABLED, getOptionalPropertyData(properties.acctDisabled()));
            localUser.setProperty<bool>(LocalUserItem::PASSWORD_NEVER_EXPIRES, getOptionalPropertyData(properties.changeLogon()));
            localUser.setProperty<QDate>(LocalUserItem::EXPIRES, QDate::fromString(getOptionalPropertyData(properties.expires()).c_str()));
            localUser.setProperty<bool>(LocalUserItem::NO_CHANGE, getOptionalPropertyData(properties.noChange()));
            localUser.setProperty<bool>(LocalUserItem::ACCOUNT_NEVER_EXPIRES, getOptionalPropertyData(properties.neverExpires()));
